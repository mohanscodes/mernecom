  ...
  const ButtonGroup = ({ next, previous }) => {
    return (
      <div className="flex justify-between items-center">
        <div className="text-xl font-bold text-[#2f2f2f]  font-serif"> {title} </div>

        <div className="flex justify-center items-center gap-3 text-[#2f2f2f] ">
          <button
            onClick={() => previous()}
            ...
            <div key={i} className="flex flex-col justify-start gap-2">
              ...
              {p.map((pl, j) => (
                ...
                <img className='w-[110px] h-[110px]' src={pl.images[0]} alt="" />
                  <div className="px-3 flex justify-start items-start gap-1 flex-col text-[#2f2f2f] ">
                  <h2>{pl.name} </h2>
